...
#define EXAMPLE_DEVICE_NAME "L2CAP_CLIENT"
#define TARGET_DEVICE_NAME "Moto"                   //Name your taget device here
#define L2CAP_DATA_LEN 50
...
static void l2cap_write_handle(void *param)
{
    int size = 0;
    int fd = (int)param;
    static const uint8_t l2cap_data[] = {0x02, 0x03, 0x020, 0x0A, 0x00, 0x06, 0x00, 0x01, 0x00, 0x02, 0x02, 0x00, 0x02, 0x02, 0x00};
    //the command to send

    const size_t l2cap_data_len = sizeof(l2cap_data) / sizeof(l2cap_data[0]);
    printf("Command sent to Scale ");
    for (size_t i = 0; i < l2cap_data_len; i++)
    {
        printf("%02x ", l2cap_data[i]);  //print sent value
    }
    printf("\n");
    do
    {
        ...
        else if (size == 0)
        {
            ...
            printf("write fail due to ringbuf is full, retry after 500 ms");
        }
        else
        {
...
static void esp_hdl_bt_l2cap_cb_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_BT_L2CAP_START_EVT:
        ...
        {
            ESP_LOGW(L2CAP_TAG, "ESP_BT_L2CAP_START_EVT: status:%d", l2cap_param->start.status);
        }
        break;
    case ESP_BT_L2CAP_SRV_STOP_EVT:
        ESP_LOGE(L2CAP_TAG, "ESP_BT_L2CAP_CLOSE_EVT: status:%d, psm = 0x%x", l2cap_param->srv_stop.status, l2cap_param->srv_stop.psm);
        break;
    default:
        break;
    }
