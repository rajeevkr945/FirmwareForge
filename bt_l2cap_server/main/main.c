...
#define EXAMPLE_DEVICE_NAME "Moto"
...
static void l2cap_read_handle(void *param)
{
    ...
    { /* The frequency of calling this function also limits the speed at which the peer device can send data. */
        ...
        {
            ESP_LOGI(L2CAP_TAG, "fd = %d data_len = %d", fd, size); // for the loging of incomming data
...
static void esp_hdl_bt_l2cap_cb_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_BT_L2CAP_OPEN_EVT:
        if (l2cap_param->open.status == ESP_BT_L2CAP_SUCCESS)
        {
            ...
            printf("OPEN EVENT");
            printf("ESP_BT_L2CAP_OPEN_EVT: status:%d\n", l2cap_param->open.status);
            printf("fd = %d\n", l2cap_param->open.fd);
            printf("tx mtu = %" PRIu32 "\n", l2cap_param->open.tx_mtu);
            printf("remote_address:%s\n", bda2str(l2cap_param->open.rem_bda, bda_str, sizeof(bda_str)));
        }
        else
        {
